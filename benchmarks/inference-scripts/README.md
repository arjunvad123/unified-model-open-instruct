...
Loads Stage 1 and tests 20 queries (5 per expected action):
- `<ACT:GEN>` — open-ended generation queries
- `<ACT:RET>` — retrieval/search queries
- `<ACT:TOOL>` — tool-use queries (weather, calculation)
- `<ACT:CODE>` — code generation queries

Prints per-category accuracy and a full confusion matrix.
